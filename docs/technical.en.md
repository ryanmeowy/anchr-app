...
| **Evidence-first answers** | Traditional RAG context resolution and keyword extraction (keywords for text recall, full questions for embeddings and reranking), fixed retrieval and answer generation; whole retrieved segments grouped by asset (up to 5 assets, 3 segments per asset, 10 total, a 24,000-code-point evidence-region budget, no neighboring expansion); hierarchical citations and horizontally scrollable source cards, follow-ups, segment previews, and source context. |
