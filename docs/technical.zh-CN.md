...
| **证据优先回答** | 传统 RAG 的完整问题补全与关键词提取（文本用关键词，向量与重排用完整问题）、固定检索与回答流程；按 asset 聚合完整命中证据（最多 5 文档/每文档 3 段/总 10 段，证据区 24000 字符预算，不扩邻近片段）；双层引用校验、横向来源卡片、追问建议、Segment 预览和原文上下文恢复。 |
...
传统 RAG 在检索和证据选择后，先用独立模型请求检查资料，再生成回答。Agent 搜索与读文档工具在返回资料前检查；异步总结按批检查原文。检查覆盖正文、标题和摘要，程序按段落判定保留原文，并记录来源和处理结果。检查失败时，该批资料停止进入后续模型。

检查复用当前 GENERATION 模型配置，增加一次或多批模型调用的延迟与用量。源文件和 Elasticsearch 保存原始资料，引用保留原文定位。检查机制及效果评估见[模型输入证据检查](evidence-cleaning.md)。
