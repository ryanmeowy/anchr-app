...
/**
 * DOCX parser that preserves heading and table location in sourceRef.
 */
@Component
@Order(Ordered.HIGHEST_PRECEDENCE + 30)
@RequiredArgsConstructor
public class DocxTextParser implements TextParserPort {

    private final TextAssetContentLoader contentLoader;

    @Override
    public boolean supports(TextAssetMetadata metadata) {
        return TextParserSupport.matchesExtension(metadata, "docx")
    ...
    public TextParseResult parse(TextAssetMetadata metadata) {
        ...
        try {
            return new TextParseResult(StructuredTextParserSupport.parseDocx(content, metadata.getObjectKey()), name());
        } catch (IOException e) {
            throw new BusinessException(ApiError.TEXT_PARSE_FAILED, "Failed to parse DOCX asset", e);
        }
    }
