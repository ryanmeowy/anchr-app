...
public class SearchController {
    ...
    @PostMapping("/kb")
    public Result<SearchTopNDTO> searchKb(@Valid @RequestBody SearchQueryDTO query) {
        String userQuery = query.getQuery();
        SearchRewriteResult rewrite = searchQueryRewriteService.rewrite(userQuery);
        RetrievalTopNQuery retrievalQuery = searchRestAssembler.toTopNQuery(query, rewrite);
        RetrievalTopNResult retrievalResult = retrievalTopNQueryApi.query(retrievalQuery);
        ...
        searchActivityAcl.recordSearchExecuted(
                query, retrievalResult.items().size());
        SearchTopNDTO page = searchRestAssembler.toTopNDto(
